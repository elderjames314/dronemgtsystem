...
public class GlobalExceptionHandler {
        ...
        private final int NOT_FOUND_CODE = HttpStatus.NOT_FOUND.value();
        ...
        public ResponseEntity<Response<?>> handleNotFoundException(NotFoundException exception) {
                ErrorResponse errorResponse = ErrorResponse.builder()
                                .code(String.valueOf(NOT_FOUND_CODE))
                                .description(exception.getMessage())
                                .build();
                Response<?> response = Response.getResponse(null, errorResponse, false, DroneConstant.NOT_FOUND,
                                NOT_FOUND_CODE);
                return ResponseEntity.status(NOT_FOUND_CODE).body(response);
        }
